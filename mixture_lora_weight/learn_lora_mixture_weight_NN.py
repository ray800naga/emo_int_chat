...
# 2層のニューラルネットワークでScaling_weightsを学習するクラス
class ScalingWeightsNN(nn.Module):
    def __init__(self, input_dim=8, hidden_dim=6, output_dim=8):
        super(ScalingWeightsNN, self).__init__()
        # 2層のニューラルネットワーク
        self.fc1 = nn.Linear(input_dim, hidden_dim)  # 隠れ層
        self.fc2 = nn.Linear(hidden_dim, output_dim)  # 出力層

    def forward(self, intent_vector):
        x = F.relu(self.fc1(intent_vector))  # 隠れ層にReLU活性化関数
        scaling_weights = F.softmax(self.fc2(x), dim=-1)  # 出力層はSoftmaxで正規化
...
# 学習ループの定義
def train_model(model, tokenizer, intent_predict_model, intent_analyze_model, dataset, optimizer, scaling_weights_nn, epochs=3, batch_size=8):
    try:
        # 学習ループ
        for epoch in range(epochs):
            ...
            for batch_idx, batch_head_index in enumerate(tqdm(range(0, dataset_num, batch_size), desc=f"Epoch {epoch + 1}/{epochs}")):
                ...
                scaling_weights = torch.squeeze(scaling_weights_nn(batch_intent_vectors))
            ...
            # モデルをエポックごとに保存
            model_save_path = f"/workspace/Emotion_Intent_Chat/emo_int_chat/mixture_lora_weight/tuned_model/lora_mixture_weight_{current_time}/{epoch + 1}.pth"
            torch.save({
                'epoch': epoch+1,
                'model_state_dict': scaling_weights_nn.state_dict(),
...
# ScalingWeightsNNの初期化
scaling_weights_nn = ScalingWeightsNN(input_dim=8, hidden_dim=6, output_dim=8).to(device)

# オプティマイザの設定
optimizer = torch.optim.Adam(scaling_weights_nn.parameters(), lr=1e-3)
...
wandb.init(
    ...
    name=f"lora_mixture_weight_{current_time}",
)
...
train_model(
    ...
    dataset=dataset,
    optimizer=optimizer,
    scaling_weights_nn=scaling_weights_nn,
    epochs=10,  # 学習エポック数
    batch_size=8  # バッチサイズ
)
