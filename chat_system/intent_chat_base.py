...
df_proposed = pd.DataFrame(columns=["messages", "output"])
...
for conversation in tqdm(test_ds):
    ...
    # dataframeに追加
    new_row = pd.DataFrame({"messages": [conversation["conversation"]], "output": [decoded[0][query_len:]]})
    df_proposed = pd.concat([df_proposed, new_row], ignore_index=True)
    
df_proposed.to_excel("df_base_output_more_stable.xlsx", index=False)
